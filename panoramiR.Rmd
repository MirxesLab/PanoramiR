...
The threshold Ct value is based on QC data for each specific batch of PanoramiR kit (`r min.up.limit` ~ `r cutoff.max`). miRNAs in each samples with Ct values less than it corresponding threshold are selected for later analysis; Expression figures below shows the number of detected miRNAs in each samples.
...
Figure below shows the miRNA expression level in each sample. Values have been filtered based on Ct value thresholds and then spike-in normalized.
...
The waterfall plot displays statistically significant (p <0.05) dCt values between the treatment and control groups. The red bars represent miRNA whose dCt greater than or equal to 1, and the blue bars represent miRNA whose dCt less than or equal to 1.
