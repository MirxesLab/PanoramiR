...
arg.piprline = 'PanoramiR'
```


# Introduction
MicroRNAs (miRNAs), a group of small non-coding RNAs, are found in many species of eukaryotes. MiRNAs regulate gene expression at the post-transcriptional level in the cells and mediate intercellular communications when secreted into the local milieu or circulation. Dysregulation of miRNAs are implicated in pathologies of many human diseases, including cancer, cardiovascular disease and other chronic diseases^[Krol, J., Loedige, I., & Filipowicz, W. (2010). The widespread regulation of microRNA biogenesis, function and decay. Nat Rev Genet, 11(9), 597-610.], where they function as signalling messengers to mediate cell-cell communication ^[Hayes, J., Peruzzi, P. P., & Lawler, S. (2014). MicroRNAs in cancer: biomarkers, functions and therapy. Trends Mol Med, 20(8), 460-469.]. Therefore, miRNAs hold great promise as biomarkers of human diseases^[Chakraborty, C., Sharma, A. R., Sharma, G., Doss, C. G. P., & Lee, S.-S. (2017). Therapeutic miRNA and siRNA: Moving from Bench to Clinic as Next Generation Medicine. Molecular Therapy - Nucleic Acids, 8, 132-143.].
...
```{r fig.align='center', fig.cap = 'Number of non-missing values of miRNA across samples'}
...
Global normalization ^[Mestdagh, P., Van Vlierberghe, P., De Weer, A., Muth, D., Westermann, F., Speleman, F., & Vandesompele, J. (2009). A novel and universal method for microRNA RT-qPCR data normalization. Genome Biol, 10(6), R64.] is performed following spike-in normalization. A normalization constant is added to the Ct values for each miRNA in the sample so that the arithmetic mean Ct values of all the samples becomes identical.\
...
```{r fig.align='center', fig.cap= "miRNA Ct values distribution of samples (Comparison 1)"}
if ("Comparison 1" %in% comparisons) {
  fig.violin.1 = fun.plot.violin('Comparison 1', col.compare.1)
  fig.violin.1
}
```

### `r if('Comparison 2' %in% comparisons) {comparisons[2]}` {.unnumbered}
```{r echo = FALSE, warning= FALSE, message=FALSE, fig.align='center', fig.cap= "miRNA Ct values distribution of samples (Comparison 2)"}
if ("Comparison 2" %in% comparisons) {
  fig.violin.2 = fun.plot.violin('Comparison 2', col.compare.2)
  fig.violin.2
}
```

### `r if('Comparison 3' %in% comparisons) {comparisons[3]}` {.unnumbered}
```{r echo = FALSE, warning= FALSE, message=FALSE, fig.align='center', fig.cap= "miRNA Ct values distribution of samples (Comparison 3)"}
if ("Comparison 3" %in% comparisons) {
  fig.violin.3 = fun.plot.violin('Comparison 3', col.compare.3)
  fig.violin.3
}
```
...
```{r fig.align='center', fig.cap = 'PCA analysis for Comparison 1'}
if('Comparison 1' %in% comparisons) {
...
### miRNA {.unnumbered}
```{r echo = FALSE, warning=FALSE, message=FALSE, fig.align='center', fig.cap='Clustergram analysis'}
p.heatmap.all = fun.heatmap(allmiRNA, "Sample-miRNA", df.anno.all, color.anno.all)
```

### Sample-sample distance {.unnumbered}
```{r echo = FALSE, warning= FALSE, message= FALSE, fig.align='center', fig.cap='Sample-to-sample distance'}
...
### `r if('Comparison 1' %in% comparisons) {comparisons[1]}` {.unnumbered}
```{r echo = FALSE, message=FALSE, warning=FALSE, fig.cap="Scatter Plot for Comparison 1", fig.align='center'}
if('Comparison 1' %in% comparisons) {
...
### `r if('Comparison 2' %in% comparisons) {comparisons[2]}` {.unnumbered}
```{r echo = FALSE, message=FALSE, warning=FALSE, fig.cap="Scatter Plot for Comparison 2", fig.align='center'}
if('Comparison 2' %in% comparisons) {
...
### `r if('Comparison 3' %in% comparisons) {comparisons[3]}` {.unnumbered}
```{r echo = FALSE, message=FALSE, warning=FALSE, fig.cap="Scatter Plot for Comparison 3", fig.align='center'}
if('Comparison 3' %in% comparisons) {
...
miRNAs found differentially expressed between the sample groups (fold change \>= `r threshold.DE.dCt` and p-value =\< `r threshold.DE.pvalue`) are subjected to gene target analysis using mirTarBase ^[ Huang, H.-Y., Lin, Y.-C.-D., Li, J., Huang, K.-Y., Shrestha, S., Hong, H.-C., Tang, Y., Chen, Y.-G., Jin, C.-N., Yu, Y., Xu, J.-T., Li, Y.-M., Cai, X.-X., Zhou, Z.-Y., Chen, X.-H., Pei, Y.-Y., Hu, L., Su, J.-J., Cui, S.-D., . . . Huang, H.-D. (2019). miRTarBase 2020: updates to the experimentally validated microRNA–target interaction database. Nucleic Acids Research, 48(D1), D148-D154.]. A subset of the gene targets is shown below. For a list of all genes targeted by the differentially expressed miRNAs, please refer to the `target_gene_n.csv` file (where n is the comparison number).
...
## GO Enrichment Analysis ^[Alexa A, Rahnenfuhrer J (2022). topGO: Enrichment Analysis for Gene Ontology. R package version 2.48.0.] {.tabset .tabset-pills} 
```{r echo = FALSE, warning=FALSE, message=FALSE}
