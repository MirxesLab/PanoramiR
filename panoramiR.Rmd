...
The threshold Ct value is `r min.up.limit` ~ `r cutoff.max` (The threshold Ct value is variable based on miRNA). miRNAs in each samples with Ct values less than it corresponding threshold are selected for later analysis; Expression figures below shows the number of detected miRNAs in each samples.
...
Figure below shows the miRNA expression levels in each samples. Values have been filtered based on Ct value thresholds and then spike-in normalized.
