...
2.As reverse transcription control. miRNA spike-ins are added to RT master mix. Measured levels of miRNA spike ins allow normalization between RT reactions to detect inconsistencies between samples e.g. presence of RT-qPCR inhibitors in some samples. If RT spike-in has Ct value greater than `r threshold.sp`, it indicates RT reaction has failed, and corresponding samples are removed`r description`.
...
p.hp.all = fun.heatmap(allmiRNA, "Sample-miRNA", df.anno.all, color.anno.all)
```
...
p.hp.s2s = pheatmap(sample.dist.matrix,
...
```{r results='hide'}
ggsave(filename = file.path(dir.out.fig, 'Heatmap_All.png'), 
       plot = p.hp.all,
       width = 10, height = 8, 
       device = 'png', dpi = 300, units = 'in')

ggsave(filename = file.path(dir.out.fig, 'Heatmap_s2s.png'), 
       plot = p.hp.s2s, width = 10, height = 8, 
       device = 'png', dpi = 300, units = 'in')
