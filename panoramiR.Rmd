...
# Preparation
source('scripts/config_change.R')
source('scripts/config.R')
source('scripts/step1_ReadInSamplesheet.R') # Read in Sample sheet
...
1. As RNA isolation control. miRNA spike-ins are added to sample lysis buffer or RNA isolation buffer before mixing with biological sample. A difference in the measured levels of miRNA spike-ins indicates varying RNA isolation yield and/or RNA purity.

2.As reverse transcription control. miRNA spike-ins are added to RT master mix. Measured levels of miRNA spike ins allow normalization between RT reactions to detect inconsistencies between samples e.g. presence of RT-qPCR inhibitors in some samples. **If RT spike-in has Ct value greater than `r threshold.sp`, it indicates RT reaction has failed**, and corresponding samples are removed (Please refer to the `SP_Check_Summary.xlsx` for details).
...
Figure below shows the miRNA expression level in each sample. Values have been filtered based on Ct value threshold (`r cutoff.min` ~ `r cutoff.max`) and then spike-in and global normalized. 
...
## miRNA Expression Level in Each Sample {.tabset .tabset-pills}
...
### miRNA {.unnumbered}
```{r fig.cap='Clustergram analysis (row-wise scaling)'}
...
if("Comparison 1" %in% comparisons) {
    write.csv(ls.diff.1$res.filter, file.path(dir.out.tbl, 'DE_analysis_1.csv'))
}

if("Comparison 2" %in% comparisons) {
    write.csv(ls.diff.2$res.filter, file.path(dir.out.tbl, 'DE_analysis_2.csv'))
}

if("Comparison 3" %in% comparisons) {
    write.csv(ls.diff.3$res.filter, file.path(dir.out.tbl, 'DE_analysis_3.csv'))
}
...
Figures below show the dCt value between treatment and control group. The red points represent miRNA whose dCt greater than or equal to `r threshold.DE.dCt`, and the blue points represent miRNA whose dCt less than or equal to - `r threshold.DE.dCt`. \
