...
# Workflow Overview
352 miRNAs are detected by MiRXES's ID3EAL™ miRNA qPCR solution [@RN4].
...
IPCs are DNA template that is run 4 times for each cDNA pool. First check the standard deviation of Cts of IPCs for each cDNA pool; Standard deviation greater than 0.5 indicates pipetting error, and caution should be taken when interpreting results. IPC can be used as positive controls for qPCR process. **If IPC has Ct value greater than 25, it indicates qPCR reaction has failed**, and corresponding samples are removed (Please refer to the `IPC_Check_Summary.xlsx` for details). Secondly, IPC can be used to normalize plate to plate variation. Due to machine variations, Ct values from different machines are not directly comparable. Use IPCs to normalize the machine to machine variation as IPCs will have the same Ct value across machines if the machine variation is eliminated.
```{r}
...
## Detected miRNA Expression Level in Each Samples
...
The top 10 miRNAs having the smallest p value between the sample groups are listed in the table below. dCt of each miRNA is expressed as the mean Ct value of that in Group A minus the mean Ct value of that in Group B. Statistical significance for each dCt is expressed as FDR corrected p-values.
...
## Genes Targeted by Selected miRNA 
