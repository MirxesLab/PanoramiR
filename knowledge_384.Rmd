...
# Introduction
MicroRNAs (miRNAs), a group of small non-coding RNAs, are found in many species of eukaryotes. MiRNAs regulate gene expression at the post-transcriptional level with in the cells and mediate intercellular communications when secreted into the local milieu or in circulation. It has been widely reported that dysregulation of miRNAs are implicated in pathologies of many human diseases, including cancer, cardiovascular disease and other chronic disease^[Krol J et al. The widespread regulation of microRNA biogenesis, function and decay. Nat Rev Genet 2010;11(9):597–610.], where they function as signalling messengers to mediate cell-cell communication ^[Hayes J et al. MicroRNAs in cancer: biomarkers, functions and therapy. Trends Mol Med 2014;20(8):460–469.]. Therefore, miRNAs hold great promise as potential biomarkers of human diseases^[Chakraborty C et al. Therapeutic miRNA and siRNA: Moving from Bench to Clinic as Next Generation Medicine. Mol Ther Nucleic Acids 2017;8:132–143.].

MiRNA expression analysis is an invaluable tool for biomarker development, which can provide important insight into the underlying mechanisms of disease development and can be translated into early diagnosis, risk stratification, as well as therapeutic management tools. MiRXES scientists and their collaborators have measured and analyzed miRNAs in over 50,000 different samples across more than 20 different diseases and conditions using the proprietary and patented qPCR technology. Based on the extensive experience in the field of miRNA detection and analysis, MiRXES can offer the most comprehensive microRNA technology platform which can translate microRNA research into clinical applications, from biomarker discovery and validation to product development. 
...
miRNAs in each groups with Ct value less than `r cutoff.max` are selected for later analysis; Ct value > `r cutoff.max` is considered as non-detect. Expression figures below shows the overview information of detected miRNAs in each samples.
...
## Detected miRNA Expression Levels in Each Samples {.tabset .tabset-pills}
Figure below shows the miRNA expression levels in each samples. Values have been filtered based on Ct value threshold (`r cutoff.max`) and then spike-in normalized.
...
The miRNAs with missing values in more than 10% (> 10%) of the total number of samples are not analysed. Else, the missing values are imputed by the lowest Ct value across the samples.\
